...
const CanDetailModal: React.FC<CanDetailModalProps> = ({ can, onClose, onEdit, onDelete, onViewImage }) => {
  const InfoRow = ({ label, value, icon, fullWidth = false }: { label: string, value: string | undefined, icon: string, fullWidth?: boolean }) => (
    <div className={`flex flex-col border-b border-gray-50 pb-3 ${fullWidth ? 'col-span-2' : ''}`}>
      <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
        <span>{icon}</span> {label}
      </p>
      <p className={`text-base font-bold text-slate-800 ${fullWidth ? '' : 'truncate'}`}>{value || '---'}</p>
    </div>
  );
      ...
      <div className="bg-white w-full h-full sm:h-auto max-w-5xl sm:rounded-[48px] shadow-2xl overflow-y-auto md:overflow-hidden flex flex-col md:flex-row sm:max-h-[95vh] animate-in zoom-in duration-300">
        ...
        <div className="md:w-1/2 flex flex-col md:overflow-hidden">
          <div className="p-4 sm:p-10 border-b border-gray-50 flex justify-between items-start shrink-0">
             <div>
                <p className="text-indigo-600 font-black text-[10px] sm:text-xs uppercase tracking-[3px] mb-1">{can.group}</p>
                <h2 className="text-xl sm:text-4xl font-black text-slate-900 tracking-tighter">{can.brand}</h2>
             </div>
          ...
          <div className="flex-1 md:overflow-hidden p-4 sm:p-10 space-y-4 sm:space-y-8">
            <div className="grid grid-cols-2 gap-3 sm:gap-8">
               <InfoRow label="Nome da Bebida" value={can.name} icon="🏷️" />
               <InfoRow label="Descrição da Lata" value={can.description} icon="📝" />
