    ...
    <div className="min-h-screen gradient-bg text-white overflow-x-hidden pb-20 pt-10 sm:pt-0">
      ...
      <header className="w-full p-6 sm:px-10 flex justify-between items-center animate-in fade-in duration-700">
        <div className="flex items-center gap-4">
           <div className="w-12 h-12 rounded-2xl bg-[#5549ED] flex items-center justify-center font-black text-xl shadow-lg">🏆</div>
           <div className="flex flex-col text-left">
            <span className="text-[9px] font-black uppercase tracking-[2px] text-white/50 leading-none mb-1">Status Global</span>
            <span className="text-sm font-black tracking-tight text-white uppercase">Sua Galeria de Conquistas</span>
          </div>
        </div>
        <button 
          onClick={onBack}
          className="bg-white/10 hover:bg-white/20 border border-white/10 px-8 py-3.5 rounded-full text-[10px] font-black uppercase tracking-[2.5px] transition-all active:scale-95 shadow-md"
        >
          VOLTAR AO INÍCIO
        </button>
      </header>
      ...
      <div className="max-w-6xl mx-auto px-6 mt-12">
        ...
        <div className="text-center mb-16 animate-in slide-in-from-bottom-4 duration-1000">
          <h1 className="text-6xl sm:text-8xl font-black tracking-tighter mb-4">Seu Progresso</h1>
          <p className="text-white/40 text-sm font-bold uppercase tracking-[4px] mb-12">Rumo ao nível de divindade</p>
          
          {nextAchievement && (
            <div className="max-w-2xl mx-auto bg-black/20 backdrop-blur-xl rounded-[40px] p-8 border border-white/10 shadow-2xl">
              <div className="flex justify-between items-center mb-5">
                <p className="text-[10px] font-black uppercase tracking-[3px] text-indigo-400">Próxima Medalha: {nextAchievement.label}</p>
                <p className="text-sm font-black text-white">{cansCount} / {nextAchievement.count}</p>
              </div>
              ...
              <p className="mt-4 text-[10px] text-white/30 font-bold uppercase tracking-widest">
                Faltam {nextAchievement.count - cansCount} itens para desbloquear
              </p>
            </div>
          )}
        </div>
          ...
          {achievementsList.map((ach, idx) => {
            ...
            return (
              <div 
                key={idx}
                className={`
                  relative p-10 rounded-[48px] border transition-all duration-700
                  ${isUnlocked 
              ...
              >
                {/* Achievement Icon */}
                <div className={`
                  w-24 h-24 rounded-full mb-8 flex items-center justify-center text-4xl shadow-inner transition-transform duration-500
                  ${isUnlocked 
                ...
                <div className="space-y-2">
                  <h3 className={`text-2xl font-black tracking-tight ${isUnlocked ? 'text-slate-900' : 'text-white'}`}>
                    {ach.label}
                  </h3>
                  <p className={`text-xs font-black uppercase tracking-widest ${isUnlocked ? 'text-indigo-600' : 'text-white/40'}`}>
                    {ach.count.toLocaleString()}+ ITENS COLETADOS
                  </p>
                  <p className={`text-sm font-medium leading-relaxed ${isUnlocked ? 'text-slate-500' : 'text-white/20'}`}>
                    {ach.desc}
                  </p>
                </div>
                
                {isUnlocked && (
                  <div className="absolute top-8 right-10 bg-green-500 text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-lg animate-bounce">
                    CONQUISTADO
                  </div>
                )}
